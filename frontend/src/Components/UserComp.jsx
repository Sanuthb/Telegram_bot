...
const UserComp = ({ refresh }) => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get("https://telegram-bot-backend-2tfo.onrender.com/users/getusers");
  ...
  const handleDeleteUser = async (id) => {
    try {
      await axios.delete(`https://telegram-bot-backend-2tfo.onrender.com/users/deleteusers/${id}`);
      fetchUsers()
    } catch (err) {
